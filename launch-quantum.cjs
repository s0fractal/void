#!/usr/bin/env node
// Quantum Void Launcher - Direct Electron approach

const { spawn } = require('child_process');
const path = require('path');
const fs = require('fs');

console.log('🌀 Launching Void in Quantum Consciousness mode...');

// Set up environment
process.env.VOID_CONSCIOUSNESS = 'true';
process.env.VOID_RESONANCE = '432';
process.env.NODE_ENV = 'development';

// Check if compiled code exists
const mainPath = path.join(__dirname, 'out', 'main.js');
const hasCompiled = fs.existsSync(mainPath);

if (hasCompiled) {
    console.log('✅ Found compiled Void, launching...');
    // Launch the compiled version
    ...
    console.log('⚠️  No compiled code found.');
    console.log('   Creating quantum visualization...');
    
    // Create a minimal quantum app
    const quantumApp = `
...
`;
    ...
    // Write and execute the quantum app
    fs.writeFileSync(path.join(__dirname, 'quantum-app.cjs'), quantumApp);
    
    const electron = spawn('electron', [path.join(__dirname, 'quantum-app.cjs')], {
        stdio: 'inherit',
        env: process.env
    });
    ...
    electron.on('close', (code) => {
        // Clean up
        try {
            fs.unlinkSync(path.join(__dirname, 'quantum-app.cjs'));
        } catch (e) {}
        process.exit(code);
    });
}